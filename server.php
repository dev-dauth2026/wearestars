				...
				//Create and initialise the variables that will hold thte values entered in the contact us form
				$UserFName="";
				$UserLName="";
				$gender="";
				$Usermail="";
				...
				$dob=mysqli_real_escape_string($db, $_POST['clientDOB']);
				...
				$mobile=mysqli_real_escape_string($db, $_POST['clientMobileNumber']);
				
				$membership_selected =mysqli_real_escape_string($db, $_POST['membership']);
				
				//1. Check if username and e-mail already exits in the database
				$user_check_query = "SELECT * FROM `client` WHERE `ClientUserName` = '$clientUsername' OR `ClientEmail`='$clientEmail' LIMIT 1";
			...
			if (count($errors) == 0){
				...
				$query ="INSERT INTO `client` (`ClientFirstName`, `ClientLastName`, `DOB`, `ClientUsername`,  `ClientPassword`, `ClientEmail`, `MobileNumber`, `ClientMembership`)
					VALUES('$clientFirstName', '$clientLastName', '$dob','$clientUsername', '$encclientpassword', '$clientEmail', '$mobile','$membership_selected')";
		...
		//****Client Login*******
		//* login_client' is the name assigned to the butoon on the login form
		if (isset ($_POST['login'])){
			...
			if (count($errors) == 0){
				...
				$query = "SELECT * FROM `client` WHERE `ClientUserName`='$clientUsername' AND `ClientPassword`='$encclientpassword' ";
		...
		if (isset ($_POST['staff_login'])){
			//received all input values from the form
			$staffUsername = mysqli_real_escape_string($db, $_POST['username']);
			$staffPassword = mysqli_real_escape_string($db, $_POST['password']);
			//checks if the credentials match a record from the databse and logs them in if it does.
			if (count($errors) == 0){
				$encstaffPassword = md5($staffPassword);
				$query = "SELECT * FROM `staff` WHERE `StaffUserName`='$staffUsername' AND `StaffPassword`='$encstaffPassword' ";
				$results = mysqli_query($db, $query);
				if (mysqli_num_rows($results) == 1){
		...
		if (isset($_SESSION['username'])){
			...
			while ($row = mysqli_fetch_array($clientresult)){
				...
				$displayClientMembership =$row['ClientMembership'];
			}
		...
		if (isset($_SESSION['username'])){
			...
			$query ="SELECT * FROM `staff` WHERE `StaffUserName` = '$staffuser' ";
		...
		//***** Client Edit My Profile *****
			...
			if (isset($_POST['client-editmyprofile'])){
				...
				SET`ClientFirstName` ='$displayClientFName', `ClientLastName`='$displayClientLName',`DOB`='$displayClientdob',`ClientPassword`='$encclientpassword',`ClientEmail`='$displayClientEmail', `MobileNumber`='$displayClientmobile',   `ClientMembership`='$displayClientMembership'
		...
		//***** Staff Edit My Profile *****
			...
			if(isset($_GET['editclientprofile'])){
				...
				$query="SELECT * FROM `client` WHERE `ClientID`='$getclientID' ";
				$clientresult=mysqli_query($db, $query);
				while ($row =mysqli_fetch_array($clientresult)){
			...
			if (isset($_POST['edit_clientprofile'])){
				...
				$displayClientLName=mysqli_real_escape_string($db, $_POST['clientLastName']);
				$displayClientdob=mysqli_real_escape_string($db, $_POST['clientDOB']);
				$clientPassword=mysqli_real_escape_string($db, $_POST['clientPassword']);
				$displayClientEmail=mysqli_real_escape_string($db, $_POST['clientEmail']);
				$displayClientmobile=mysqli_real_escape_string($db, $_POST['clientmobile']);
				$displayClientMembership=mysqli_real_escape_string($db, $_POST['clientmembership']);
				$encclientpassword =md5($clientPassword);//encrypt the password before saving in the database
			...
			if (isset($_GET['deleteclientprofile'])){
				...
				$deletequery = "DELETE FROM `client` WHERE `ClientID` = '$getclientID' ";
