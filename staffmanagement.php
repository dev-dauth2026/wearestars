<?php
	...
	if (isset($_POST['search-staff'])){
		$searchinput= $_POST['staff-search-input'];
		//Search uer from input
		$searchquery = "SELECT * FROM `staff` WHERE lower(`StaffFirstName`) LIKE '%$searchinput%' OR lower(`StaffLastName`) LIKE '%$searchinput%'";
		$searchresults=mysqli_query($db, $searchquery);
	}
?>
<!DOCTYPE html>
...
<div class="header">
  ...
  <a href="staffmanagement.php?logout='1'">Log Out</a>
</div>	  
				  <div id="main">
...
<h3>Staff Search</h3>
<input type="text" name="staff-search-input"> <button type="submit" value="Search" name="search-staff" class="search">Search</button>
<button type="button" class="addstaff" value="Add Staff Member" onclick="addstaff()">ADD STAFF MEMBER</button><br><br>
  <h3><table>
  ...
  <tr>
	...
	<td><a href="edit_staff.php?editstaffprofile=<?php echo $row['StaffID'];?>">Edit</a></td>
	<td><a onclick="return confirm('Delete Profile');" href="staffmanagement.php?deletestaffprofile=<?php echo $row['StaffID'];?>">Delete</a></td>
  </tr> 
  <?php }?>
  </table></h3>
